...
impl<'a> NFAH<'a> {
    ...
    pub fn project<'b>(
        ...
        // The dimension can be set to 0, since in the projected automaton
        // we only carry `Option<String>` and do not rely on a variable dimension anymore.
        let mut projected = EpsilonNFA::new(states_arena, trans_arena, 0);

        // Map old state pointer => new state pointer
        let mut old_to_new = HashMap::with_capacity(self.states.len());
        let mut queue = VecDeque::new();

        // 1) Create new states for each initial state in `self` and queue them for BFS.
        for &init_state in &self.initial_states {
            let new_init = projected.add_state(true, init_state.is_final);
            old_to_new.insert(init_state as *const _, new_init);
            queue.push_back(init_state);
        }

        // 2) BFS over transitions; replicate each state/transition in the new EpsilonNFA.
        while let Some(old_state) = queue.pop_front() {
            let new_state = old_to_new[&(old_state as *const _)];
            let transitions = old_state.transitions.borrow();
            for &trans in transitions.iter() {
                let (ref action, old_var) = trans.label;
                let old_next = trans.next_state as *const _;

                // If next state not yet seen, create it
                if !old_to_new.contains_key(&old_next) {
                    let new_next = projected.add_state(false, trans.next_state.is_final);
                    old_to_new.insert(old_next, new_next);
                    queue.push_back(trans.next_state);
                }
                let new_next = old_to_new[&old_next];

                // Keep transitions with `var` as Some(action) else None.
                ...
                projected.add_transition(new_state, new_label, new_next);
            }
        }
