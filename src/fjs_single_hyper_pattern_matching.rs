...
impl<Notifier: ResultNotifier> FJSSingleHyperPatternMatching<'_, Notifier> {
    ...
    // Find the next valid position from the waiting queue
    fn find_next_valid_position(&mut self) -> Option<Reverse<StartPosition>> {
        while let Some(position) = self.waiting_queue.pop() {
            if self.is_valid_position(&position.0) {
                ...
                for var in 0..self.dimensions() {
                    let start_index = position.0.start_indices[var];
                    ...
                    if shortest_matching_length > 0 {
                        ...
                        let shortest_end_index = start_index + shortest_matching_length - 1;
                        let next_index = start_index + shortest_matching_length;

                        if stream.len() >= next_index - stream.start
                            && shortest_end_index < stream.len() + stream.start
                        {
                            let readable_data = stream.readable_slice();
                            let slice_shortest_end_idx = shortest_end_index - stream.start;
                            let slice_next_idx = next_index - stream.start;

                            if slice_shortest_end_idx < readable_data.len()
                                && slice_next_idx < readable_data.len()
                            {
                                let last_accepted_words =
                                    &self.quick_search_skip_value.last_accepted_word[var];

                                if !last_accepted_words
                                    .contains(&readable_data[slice_shortest_end_idx])
                                {
